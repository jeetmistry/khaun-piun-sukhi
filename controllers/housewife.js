...
exports.deleterecipe = (req, res) => {
    ...
    db.query('DELETE FROM recipe where R_id = ?', [id], (err, row) => {
        if (!err) {
            ...
            db.query(`SELECT recipe.R_id,recipe.housewife_id,recipe.Rname,recipe.Rdesc,recipe.RImage,
            recipe.Ingredients,housewife.housewife_id,housewife.h_first_name,housewife.h_last_name 
            FROM recipe join housewife ON recipe.housewife_id = HOUSEWIFE.housewife_id && recipe.housewife_id =?`, [process.env.HOUSEWIFE_ID],
                (error, rows) => {
                    if (!error) {
                        res.render('hviewrecipes', { rows });
                    } else {
